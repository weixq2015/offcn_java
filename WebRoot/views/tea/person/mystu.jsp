    ...
    <div class="main">
        ...
        <table border="1" width="100%" class="tab">
            ...
            <c:forEach var="m" items="${stulist}" varStatus="status">
                <tr>
					...
					<td>					
						<c:if test="${m.classes.type==0}">
						实验班
						</c:if>
						<c:if test="${m.classes.type==1}">
						基础班
						</c:if>
					</td>
					<td>${m.course.name }</td>
					...
					<td>
                    <a href="${pageContext.request.contextPath }/tea/addgrade/${m.id}/${m.course.id}" class="abtn">录入成绩</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
        ...
        <script type="text/javascript">
           ...
           function handlePaginationClick(new_page_index, pagination_container){
               location.href="<c:url value="/tea/mystu/${user.id}"/>?pageNO="+(new_page_index+1);
           }
